...
{
    public class WorkScanView : NetworkPageMonoBehavior<WorkScanView>
    {
        [Header("UI")]
        ...
        private async Task StartWork()
        {
            ...
            NowTime = NowTime.Substring(0, 16);
            var order = await _eqptService.GetWorkOrder(_rBuildingCode, _recordSn);
            order.StartTime = NowTime;
            order.Status = "Processing";
            order.StatusType = 1;
            order.StatusColor = "c-processing";
            order.StatusBGColor = "bgc-processing";
            var orderDevIndex = order.OrderDevices.FindIndex(dev => dev.BuildingCode == _rBuildingCode && dev.DeviceCode == _deviceCode);
            ...
            await _eqptService.SetWorkOrder(order);
        }
