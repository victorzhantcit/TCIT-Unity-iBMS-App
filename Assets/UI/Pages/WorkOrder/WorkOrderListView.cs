...
{
    public class WorkOrderListView : NetworkPageMonoBehavior<WorkOrderListView>
    {
        ...
        [SerializeField] private WorkOrderListItem _orderListItemPrefab;
        ...
        private ObjectPool<WorkOrderListItem> _orderListItemPool;
        ...
        {
            _orderListItemPool = new ObjectPool<WorkOrderListItem>(_orderListItemPrefab, _orderListContent);
        ...
        private async Task GetList(int type)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    var response = await _eqptService.PostWorkOrders(queryData);
                ...
                foreach (var o in _orders)
                {
                    ...
                    if (o.Show == true)
                    {
                        WorkOrderListItem orderView = _orderListItemPool.Get();

                        orderView.UpdateListItem(o);
                        orderView.BindEnterOrderEvent(() => _ = StartWorking(o.BuildingCode, o.RecordSn));
            ...
            {
                DebugLog(ex.Message, LogType.Exception);
            }
            finally
            {
        ...
        private void ClearOrderList()
        {
            ...
            foreach (Transform item in _orderListContent)
            {
                ...
                if (item.TryGetComponent<WorkOrderListItem>(out var itemComponent))
                {
        ...
        private void UpdateOrderList()
        {
            ...
            foreach (EqptOrder order in _orders)
            {
                if (order.Show == true)
                {
                    ...
                    orderView.BindEnterOrderEvent(() => _ = StartWorking(order.BuildingCode, order.RecordSn));
                    orderView.transform.SetSiblingIndex(siblingIndex);
                    siblingIndex++;
                }
            }
        ...
        private async Task StoragePhotos(string photoSns)
        {
            ...
            for (int photoIndex = 0; photoIndex < photoCount; photoIndex++)
            {
                ...
                var photoData = await _eqptService.GetWorkPhoto(sn);
        ...
        private async Task GetPhoto(string sn)
        {
            ...
            {
                await _eqptService.SetWorkPhoto(sn, base64Img, createTime);
                DebugLog($"Photo({sn}) request completed.");
            }
        }

        private async Task StartWorking(string buildingCode, string recordSn)
        {
            await Task.Yield();
